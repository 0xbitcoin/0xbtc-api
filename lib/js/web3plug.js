...
const contractData = require('../config/deployedcontractinfo.json')
...
module.exports =  class Web3Plug {
  ...
  constructor(web3provider){
    ...
    console.log('init web3provider with ',web3provider)
   }
  ...
  async getBlockNumber(){ 
    return await new Promise((resolve,reject) => {                    
      web3.eth.getBlockNumber(function(error,netId){
        if(error){reject(error)}
        resolve(netId);
      })
    })
  ...
  getContractDataForNetworkID(networkId){
    ...
    if(netName){
        ...
        return contractData.networks[netName].contracts
    }
